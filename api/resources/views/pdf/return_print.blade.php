...
<head>
    <title>Nota {{$order->returnNumber}}</title>
    <style>
        @font-face {
            font-family: 'Tahoma';
...
<body>
    ...
    <?php
    $totalLoop = ceil(count($order->invoiceReturnItems) / 15);
    $totalItem = count($order->invoiceReturnItems);
    $data = $order->invoiceReturnItems;
    $loopItem = 15;
    $indexRow = 0;
    $idx = 0;
    $last = false;
    ...
    ?> <table cellPadding=0 cellSpacing=0>
            <tbody>
                <tr>
                    ...
                    <td class="text-center" style="font-size:16px;vertical-align:top;padding:0px;width:27%;"><u><b>Return {{$order->typeReturn === 'CUSTOMER' ? 'Penjualan' : 'Pembelian'}}</b></u></td>
                ...
                <tr>
                    ...
                    <td colspan="4">Kepada : {{ucfirst($order->partner->name)}}</p>
                    </td>
                ...
                <tr>
                    <td>No Faktur : {{$order->returnNumber}}</td>
        ...
        <table class="table-barang" cellPadding=0 cellSpacing=0>
            ...
            <tbody>
                ...
                <?php if ($idx == $totalLoop) : ?>
                    <tr>
                        <td colspan="3" style="font-size: 10px !important;">MOHON DICEK KEMBALI SEBELUM DI TANDATANGANI KEKURANGAN SESUDAHNYA TIDAK MENJADI TANGGUNG JAWAB KAMI.</td>
                        @if($isAdmin)
